...
from src.data_reader.dataset import RawDataset, ReverseRawDataset, RawXXreverseDataset,TorchDataSet_train
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=64,
                        help='batch size')
    parser.add_argument('--audio-window', type=int, default=10240,
    ...
    parser.add_argument('--alpha-rev-cpc', default=0.3)
    parser.add_argument('--mask-input-channel', default=256, type=int)
    ...
    model = CDCK2(args.timestep, args.batch_size, args.audio_window, args.alpha_parameter)#.to(device)
    model = model.to(device)
    decom_model = FDCN(1, 3, 3, 3)
    decom_model = decom_model.to(device)
    #pdb.set_trace()
    #model = CDCK2(args.timestep, args.batch_size, args.audio_window)
    # gpus = [0,0,0,0]
    # gpus[0] = 0
    # gpus[1] = 1
    # gpus[2] = 2
    # gpus[3] = 3
    # if use_cuda:
        # model = torch.nn.DataParallel(model, gpus, gpus[0]).cuda(gpus[0])
    ...
    train_loader2 = TorchDataSet_train(args.train_raw, args.train_list, args.audio_window,args.batch_size,1,args.audio_window,False)
    validation_loader2 = TorchDataSet_train(args.validation_raw, args.validation_list, args.audio_window,args.batch_size,1,args.audio_window,False)
    # nanxin optimizer  
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train(args, model, decom_model, device, train_loader, train_loader2, optimizer_decom, optimizer, epoch, args.batch_size)
        val_acc, val_loss = validation(args, model, decom_model, device, validation_loader,validation_loader2, args.batch_size)
