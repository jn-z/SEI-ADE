...
class Executor(object):
    ...
    def load_checkpoint(self, model, optimizer, filename):
        ...
        if model is not None:
            ...
            for key in cnn_key_list:
                model_dict[key] = checkpoint['model'][key]
            cnn_key_list = [key for key in checkpoint['model'].keys() if 'header' in key]
            for key in cnn_key_list:
                model_dict[key] = checkpoint['model'][key]
    ...
    def fit(self, train_module):
        ...
        self.eval_classify_spk(self.devset)
    ...
    def eval_classify_spk(self,devset):
        ...
        with torch.no_grad():
            ...
            for bid, batch_data in enumerate(devset):
                ...
                params = [p.cuda(self.args.gpu) for p in (src, state_lab)]
                anchor_sv, predict, ce_loss,tar = self.train_module(*params, fea_frames, 0)
                import pdb
                pdb.set_trace()
                state_lab = state_lab.view(1,-1)
                num_corr_batch = int(state_lab.eq(predict.cpu()).float().sum())
                num_corr_all = num_corr_all+num_corr_batch
                num_batch = state_lab.size(dim=1)
                num_all = num_all+num_batch
                #import pdb
                #pdb.set_trace()
                state_lab2 = np.int64(state_lab.numpy()>0)
                predict2 = np.int64(predict.cpu().numpy()>0)
                state_lab2 = torch.from_numpy(state_lab2)
                predict2 = torch.from_numpy(predict2)
                num_corr2_batch = int(state_lab2.eq(predict2).sum())
                num_corr2_all = num_corr2_all+num_corr2_batch
                num_batch2 = state_lab2.size(dim=1)
                num_all2 = num_all2+num_batch2
            acc2 = num_corr2_all / num_all2
            acc = num_corr_all / num_all
            logging.info('total %d corr %d acc %.4f' % (num_all,num_corr_all,acc))
            logging.info('total %d corr %d acc2 %.4f' % (num_all2,num_corr2_all,acc2))
            #acc = num_corr_all / num_all
            #print('acc:'acc)
